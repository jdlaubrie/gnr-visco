...
lamellar_thickness_ini = 0.0030  #0.0020
...
t_adventitia = r_outer - r_interlayer
...
r_lamella[-1,1] = r_interlayer                      #outer in last lamella
for i in range(n_lamellar_layers):
    # inner radius for an elastin lamellar layer
    r_lamella[i+1,0] = r_inner + float(i+1)*unit_step
    # outer radius for an elastin lamellar layer
...
print("Lamellar radii:")
print(r_lamella)
print("Muscle radii = " + str(r_muscle))
whole_lamellar = n_lamellar_layers*lamellar_thickness
whole_interlamellar = t_media-whole_lamellar
print("Media thickness    -> {}, {}".format(t_media,1.0))
print("All lamellas       -> {}, {}".format(whole_lamellar,whole_lamellar/t_media))
print("All inter-lamellas -> {}, {}".format(whole_interlamellar,whole_interlamellar/t_media))
...
time_fibre = 30.0
growth_gral = 0.05
# media layer, lamella. total_density = 1050.0e-9
fraction_mat1 = 0.8
fraction_fib1 = 0.2
# matrix properties
E_gnd1 = 0.25
...
tau_fibre1 = time_fibre      # x3 -> 90-days
growth_fibre1 = growth_gral

# media layer, inter-lamella. total_density = 1050.0e-9
fraction_mat2 = 0.55
fraction_fib2 = 0.45
# matrix properties
E_gnd2 = 0.25
...
# remodeling properties
tau_fibre2 = time_fibre      # x3 -> 90-days
growth_fibre2 = growth_gral
...
E_gnd3 = 0.25
nu_gnd3 = 0.49
prestretch_circ3 = lambda_t
prestretch_long3 = lambda_z
# fibre properties
k1_fib3 = 0.40
k2_fib3 = 3.5
angle_fib3 = 53.0
prestretch_fib3 = lambda_f
# remodeling properties
tau_fibre3 = time_fibre      # x3 -> 90-days
growth_fibre3 = growth_gral
...
l_max = 1.40
# remodeling properties
tau_cell = 1.5        # x3 -> 4.5-days
growth_cell = growth_gral

inner_pressure1 = 0.0120
factor_p = 1.50                           # perturbation
period1 = 1.0                           # prestress step
period2 = 50.0                          # normotensive remodeling step
period3 = 1.0                           # perturbation step
period4 = 150.0                          # perturbed remodeling step
time1 = period1
time2 = time1 + period2
time3 = time2 + period3
time4 = time3 + period4
...
for i in range(n_lamellar_layers+1):
    s0.CircleByCenterPerimeter(center=(0.0, 0.0), point1=(r_lamella[i,0], 0.0))
    s0.CircleByCenterPerimeter(center=(0.0, 0.0), point1=(r_lamella[i,1], 0.0))
...
# lists of lamellar and interlamellar regions
# the intima and adventitia are included as interlamellar regions
dirty_interlamella_faces = []
dirty_lamella_faces = []
dirty_interlamella_faces_index = []
dirty_lamella_faces_index = []
for iunit in range(n_lamellar_layers+1):
    dirty_interlamella_faces.append(part1.faces.getByBoundingCylinder(
                center1=(0,0,-1.0), center2=(0,0,1.0), radius=1.001*r_lamella[iunit,0]))
    dirty_lamella_faces.append(part1.faces.getByBoundingCylinder(
                center1=(0,0,-1.0), center2=(0,0,1.0), radius=1.001*r_lamella[iunit,1]))
    index_i = []
    index_o = []
    for jface in dirty_interlamella_faces[iunit]:
        index_i.append(jface.index)
    for jface in dirty_lamella_faces[iunit]:
        index_o.append(jface.index)
    dirty_interlamella_faces_index.append(index_i)
    dirty_lamella_faces_index.append(index_o)
# include the adventitia in the list of interlamellar faces
dirty_interlamella_faces.append(all_faces)
index_i = []
for jface in dirty_interlamella_faces[-1]:
    index_i.append(jface.index)
dirty_interlamella_faces_index.append(index_i)

# cleaning of the lamellar and interlamellar lists, as they are all mixed until here
lamella_faces = []
interlamella_faces = []
#interlamella_faces.append(dirty_interlamella_faces[0][0])
for i in range(len(dirty_lamella_faces_index)):
    for iface in dirty_lamella_faces[i]:
        if not iface.index in dirty_interlamella_faces_index[i]:
            lamella_faces.append(iface)
    for iface in dirty_interlamella_faces[i+1]:
        if not iface.index in dirty_lamella_faces_index[i]:
            interlamella_faces.append(iface)

# body set
part1.Set(faces=all_faces, name='body')
# interlamellar set
part1.Set(faces=part.FaceArray(interlamella_faces[0:-1]), name='interlamella')
# elastin lamellar set
part1.Set(faces=part.FaceArray(lamella_faces), name='lamella')
# adventitia set
part1.Set(faces=part.FaceArray(interlamella_faces[-1:]), name='adventitia')
...
outer_point = part1.vertices.getByBoundingBox(
                xMin=0.999*r_outer, yMin=-0.01*r_outer, zMin=-1.0,
                xMax=1.001*r_outer, yMax=0.01*r_outer, zMax=1.0)
inner_point = part1.vertices.getByBoundingBox(
                xMin=0.999*r_inner, yMin=-0.01*r_inner, zMin=-1.0,
                xMax=1.001*r_inner, yMax=0.01*r_inner, zMax=1.0)
...
# make a list with the edges to remove
...
# section membrane
model1.HomogeneousSolidSection(name='lamella_section', material=material1_name)

# section assignment, BOTTOM_SURFACE, TOP_SURFACE
part1.SectionAssignment(region=part1.sets['lamella'], sectionName='lamella_section',
        offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='', 
        thicknessAssignment=FROM_SECTION)

#-----------------------------------------------------------------------------#
material2_name = 'ground_inter'
...
# section membrane
model1.HomogeneousSolidSection(name='inter_section', material=material2_name)

# section assignment, BOTTOM_SURFACE, TOP_SURFACE
part1.SectionAssignment(region=part1.sets['interlamella'], sectionName='inter_section',
        offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='',
        thicknessAssignment=FROM_SECTION)

#-----------------------------------------------------------------------------#
material3_name = 'ground_adven'
model1.Material(name=material3_name)
material3 = model1.materials[material3_name]
material3.UserMaterial(mechanicalConstants=(E_gnd3, nu_gnd3, k1_fib3, k2_fib3, angle_fib3,
                        prestretch_circ3, prestretch_long3, prestretch_fib3,
                        fraction_mat3, fraction_fib3, tau_fibre3, growth_fibre3))
material3.Depvar(n=23)

# section membrane
model1.HomogeneousSolidSection(name='adven_section', material=material3_name)
...
model1.ViscoStep(name='Step-1', previous='Initial', timePeriod=period1,
                  ...
                  stabilizationMethod=DISSIPATED_ENERGY_FRACTION, adaptiveDampingRatio = damp_accuracy,
                  cetol=visco_tol, integration=EXPLICIT_ONLY,
                  maxNumInc=100, initialInc=initial_inc1, minInc=1e-5, maxInc=max_inc1,
                  nlgeom=ON)
initial_inc2 = 0.10    #0.2 seems to be a good initial increment. growth and remodeling is explicit
max_inc2 = period2/20.0
model1.ViscoStep(name='Step-2', previous='Step-1', timePeriod=period2,
                  timeIncrementationMethod=AUTOMATIC,
                  stabilizationMethod=DISSIPATED_ENERGY_FRACTION, adaptiveDampingRatio = damp_accuracy,
                  cetol=visco_tol, integration=EXPLICIT_ONLY,
                  maxNumInc=1000, initialInc=initial_inc2, minInc=0.001, maxInc=max_inc2,
                  nlgeom=ON)
initial_inc3 = period3/20.0
max_inc3 = period3/4.0
model1.ViscoStep(name='Step-3', previous='Step-2', timePeriod=period3,
                  timeIncrementationMethod=AUTOMATIC,
                  stabilizationMethod=DISSIPATED_ENERGY_FRACTION, adaptiveDampingRatio = damp_accuracy,
                  cetol=visco_tol, integration=EXPLICIT_ONLY,
                  maxNumInc=100, initialInc=initial_inc3, minInc=1e-5, maxInc=max_inc3,
                  nlgeom=ON)
initial_inc4 = 0.10    #0.2 seems to be a good initial increment. growth and remodeling is explicit
max_inc4 = period4/20.0
model1.ViscoStep(name='Step-4', previous='Step-3', timePeriod=period4,
                  timeIncrementationMethod=AUTOMATIC,
                  stabilizationMethod=DISSIPATED_ENERGY_FRACTION, adaptiveDampingRatio = damp_accuracy,
                  cetol=visco_tol, integration=EXPLICIT_ONLY,
                  maxNumInc=1000, initialInc=initial_inc4, minInc=0.001, maxInc=max_inc4,
                  nlgeom=ON)
...
model1.HistoryOutputRequest(name='H-Output-2g', createStepName='Step-2',
                            region=model1.rootAssembly.allInstances['ground-1'].sets['body'],
                            variables=PRESELECT, numIntervals=int(frame_factor*period2))
model1.historyOutputRequests['H-Output-2g'].deactivate('Step-3')
model1.HistoryOutputRequest(name='H-Output-3g', createStepName='Step-3',
                            region=model1.rootAssembly.allInstances['ground-1'].sets['body'],
                            variables=PRESELECT, numIntervals=20)
model1.historyOutputRequests['H-Output-3g'].deactivate('Step-4')
model1.HistoryOutputRequest(name='H-Output-4g', createStepName='Step-4',
                            region=model1.rootAssembly.allInstances['ground-1'].sets['body'],
                            variables=PRESELECT, numIntervals=int(frame_factor*period4))
...
model1.HistoryOutputRequest(name='H-Output-2c', createStepName='Step-2',
                            region=model1.rootAssembly.allInstances['cell-1'].sets['body'],
                            variables=PRESELECT, numIntervals=int(frame_factor*period2))
model1.historyOutputRequests['H-Output-2c'].deactivate('Step-3')
model1.HistoryOutputRequest(name='H-Output-3c', createStepName='Step-3',
                            region=model1.rootAssembly.allInstances['cell-1'].sets['body'],
                            variables=PRESELECT, numIntervals=20)
model1.historyOutputRequests['H-Output-3c'].deactivate('Step-4')
model1.HistoryOutputRequest(name='H-Output-4c', createStepName='Step-4',
                            region=model1.rootAssembly.allInstances['cell-1'].sets['body'],
                            variables=PRESELECT, numIntervals=int(frame_factor*period4))
...
model1.TabularAmplitude(name='load_amp1', timeSpan=TOTAL,
        smooth=SOLVER_DEFAULT, data=((0.0, 0.0), (time1, 1.0), (time2, 1.0), (time3, factor_p), (time4, factor_p)))
...
# loads
model1.Pressure(name='pressure1', createStepName='Step-1',
...
# create job
mdb.Job(name='cylinder3_2d', model='aortic_wall', description='', type=ANALYSIS,
...
model1.keywordBlock.insert(91, """** \n** PREDEFINED FIELDS\n** \n** Name: initial_stretch Type: Solution""")
model1.keywordBlock.insert(92, """*Initial Conditions, type=SOLUTION, user""")

# write input file
mdb.jobs['cylinder3_2d'].writeInput()
